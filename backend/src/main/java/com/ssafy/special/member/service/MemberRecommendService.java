...
//    만약 다음 음식이 없는 경우 아무 처리x
    ...
    public void implicitFeedback(String memberEmail, FeedbackDto feedbackDto, Long nextFoodSeq, int googleSteps, String weather) throws Exception {
        boolean isLast = nextFoodSeq.equals(0L) ? true: false;
        ...
        List<MemberRecommend> memberRecommend = getMemberRecommends(memberSeq, nowFood.getFoodSeq());

        lastFoodRating = getLastFoodRating(member, memberSeq, nowFood, feedback, lastFoodRating, memberRecommend);

        if(feedback == 4) addNowFoodToFavorite(member, memberSeq, nowFood);

//        피드백 반영
//        패스할때 이전에 상세보기였는지, 좋아요였는지,
//        언제 추천받았고 패스를 한건지 시간을 고려해서 패스 반영
        addFeedback(weather, member, nowFood, feedback, lastFoodRating, totalSteps, memberRecommend);


//        다음에 볼 음식을 추천받은 리스트에 추가하기
//        상세보기, 좋아요가 아니면서 마지막 추천 음식이 아닌 경우
        addNotBadFoodToRecommendList(nextFoodSeq, weather, isLast, member, feedback, totalSteps);

    }

    private int getLastFoodRating(Member member, Long memberSeq, Food nowFood, int feedback, int lastFoodRating, List<MemberRecommend> memberRecommend) {
        if(!memberRecommend.isEmpty()) lastFoodRating = memberRecommend.get(0).getFoodRating();

        if (feedback == 0) {
            lastFoodRating = getLastFoodRating(member, memberSeq, nowFood);
        } else if (feedback == 2) {
            lastFoodRating = 2;
            log.info("패스");
        } else if (feedback == 3) {
...
//                  좋아하는 음식이지만 현재는 패스 -> 좋아한지 일주일 넘었으면 새로운 데이터 추가
    ...
    private void addNotBadFoodToRecommendList(Long nextFoodSeq, String weather, boolean isLast, Member member, int feedback, int totalSteps) {
    ...
    private int getLastFoodRating(Member member, Long memberSeq, Food nowFood) {
        int lastFoodRating;
        lastFoodRating = 0;
        log.info("차단");
//           현재 음식을 차단 리스트에 추가
